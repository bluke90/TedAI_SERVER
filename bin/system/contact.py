...
class Contact:
    ...
    def __init__(self, weather, user):
        ...
        self.sendnumber = self.parser.get('twilioSettings', 'sendNumber'); self.usernumber = self.parser.get('twilioSettings', 'usernumber')
        self.account_sid = self.parser.get('twilioSettings', 'account_sid'); self.auth_token = self.parser.get('twilioSettings', 'auth_token')
        self.client = Client(self.account_sid, self.auth_token)
        self.weather = weather; self.user = user
        ...
        if resp_num == self.usernumber:
            if 'weather' in resp:
                ...
                if operator in resp:
                    ...
                    self.send_message(data, self.user.userdata['userdata']['phone_number'])
                    return
            elif 'going' in resp:
                self.send_message('Okay Blake, i will change youre current location accordingly.')
                resp_split = resp.split(' ')
            ...
            if profile is not None:
                ...
                self.send_message(foward_msg, self.user.userdata['userdata']['phone_number'])
